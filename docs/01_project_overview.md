...
「Tsunagu（ツナグ）」は、日本語の「繋ぐ（つなぐ）」を由来とし、人（採用担当者・候補者・紹介者）と情報・プロセスを繋ぎ、採用のプロセスを円滑にするサービスです。
...
- **主要ペルソナ（`docs/08_requirements.md` セクション1）**
  - **兼務採用担当（SMB）**:
    - *デモグラフィック*: 従業員1〜100名の企業で人事・総務・経理を兼務する30〜45歳の女性・男性。採用専門はなく、週に10-20時間程度の担当業務。
    - *行動特性*: Excel/スプレッドシートで求人管理、複数の求人媒体に手動で投稿、Slack/LINEで候補者コミュニケーション。成果報酬の計算は手動でミスがち。
    - *ペインポイント*: 「AIが何でその評価をしたかわからない」「決済手続きが手間で営業成約が先延ばし」「候補者重複に気づかず無駄な時間を使う」。
    - *目標*: 採用業務時間を大幅削減、ミス無しで信頼できる採用、自動化で本来業務（営業・生産性）に集中。
    - *Tsunaguへの期待*: テンプレート求人作成、自動化媒体投稿、LINE通知、説明可能なスコア、重複検知、自動成果報酬計算・決済。
  - **スタートアップCTO/採用責任者**:
    - *デモグラフィック*: 従業員50〜200名のテック系スタートアップのCTOまたは人事部長。35〜50歳、エンジニア/PM出身でデータドリブン思考。
    - *行動特性*: 自社クラウドで求人管理、複数のATS試用経験、アジャイル採用プロセス。週5日のうち2-3日を採用業務に割く。
    - *ペインポイント*: 「AIの判断理由が不明瞭で意思決定できない」「海外拡大でカルチャーマッチ重視」「導入にかかる時間が長く、ROI算出が難しい」。
    - *目標*: 優秀人材の高速採用、バイアスフリーの客観的評価、エンジニアカルチャーへのフィット予測、決済・紹介料の自動管理。
    - *Tsunaguへの期待*: Explainable AIスコア、カルチャーマッチング、Google Calendar統合、Stripe決済自動化、リアルタイム採用ダッシュボード。
  - **紹介エージェント**:
    - *デモグラフィック*: 人材紹介会社（SES/エージェント企業）の営業担当またはマネージャー。25〜40歳、営業出身で成果報酬制。
    - *行動特性*: 複数企業・求人管理、候補者面談・進捗報告、成果報酬の計算・請求。平均で月10-15件の成約を目指す。
    - *ペインポイント*: 「進捗共有の手間がかかり年齢層」「成果報酬の遅延や未払いが信頼を損なう」「他社比較する候補者の比較を透明化したい」。
    - *目標*: 成約率向上、信頼性高い報酬管理、候補者体験の向上でリピート紹介増加。
    - *Tsunaguへの期待*: エージェント専用ポータル、LINE/Chatwork通知、Stripe決済の即時分配、AIマッチングスコア、採用プロセス透明化。
